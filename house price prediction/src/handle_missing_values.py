...
class DropMissingValuesStrategy(MissingValueHandlingStrategy):
    ...
    def handle(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        logger.info(f"Dropping missing values with axis = {self.axis} and thresh = {self.thresh}"
        )
...
class FillMissingValuesStrategy(MissingValueHandlingStrategy):
    ...
    def handle(self, df: pd.DataFrame)-> pd.DataFrame:
        ...
        df.cleaned = df.copy()
        if self.method == 'mean':
            numeric_columns= df_cleaned.selectdtypes(include='number').columns
            df_cleanead[numeric_columns] = df_cleaned[numeric_column].fillna(df[numeric_columns].mean()
            )
            ...
            logger.info(f"Unknown method '{self.method}'. No missing values handled.")
